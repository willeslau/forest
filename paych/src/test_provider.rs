...
pub struct TestPaychProvider<BS> {
    //	lk           sync.Mutex
...
impl<BS: BlockStore + Send +Sync + 'static> TestPaychProvider<BS> {
    pub fn new(bs: BS) -> Self{ TestPaychProvider {
        bs: bs,
    ...
    pub async fn set_account_addr(&self, a: Address, lookup: Address) {
        self.account_state.write().await.insert(a, lookup);
    }
    pub async fn set_paych_state(&self, a: Address, state: (ActorState, PaychState)) {
        self.paych_state.write().await.insert(a, state);
    }
    pub async fn set_call_response(&self, resp: InvocResult) {
...
impl<BS: BlockStore + Send + Sync + 'static> PaychProvider<BS> for TestPaychProvider<BS> {
    ...
    fn get_paych_state(
        ...
        match st {
            ...
            None => Err(format!("cant find address when resolving paych state {:?}", addr).into()),
        }
    }
